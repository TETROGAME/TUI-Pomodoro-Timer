...
class AudioManager:
    ...
    def _callback(self, outdata, frames, time_info, status) -> None:
        ...
        for voice in self._voices:
            ...
            if voice.current_gain != voice.target_gain:
                diff = voice.target_gain - voice.current_gain
                step = diff / max(to_read, 1)
                gains = np.linspace(voice.current_gain, voice.target_gain, to_read)
                voice.current_gain = voice.target_gain

                if (
                    voice.current_gain == 0.0
                    and voice.on_fade_complete == FadeAction.REMOVE
                ):
                    voices_to_remove.append(voice)
                    continue
                elif (
                    voice.current_gain == 0.0
                    and voice.on_fade_complete == FadeAction.FREEZE
                ):
                    voice.frozen = True
                    continue
            else:
    ...
    def pause(self) -> None:
        self._commands.put(("set_paused", True))
